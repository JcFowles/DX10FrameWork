...
float3 RandUnitVec3(float offset)
{
	...
	// coordinates in [-1,1]
	float3 v = (float3)g_RandomTexure.SampleLevel(gTriLinearSam, u, 0);
...
{	
	...
	if( gIn[0].type != PT_EMITTER )
	{
		...
		float4x4 ViewProj = mul(g_matView, g_matProj);
...
{
	float arrayIndex = 0;
	return g_TexureArray.Sample(gTriLinearSam, float3(pIn.texC, arrayIndex))*pIn.color;
}
